...
RSCOutcomes['GA_FNAME'] = (
        RSCOutcomes['GA_FNAME'].fillna('') +
        ' ' + 
         RSCOutcomes['GA_LNAME']
         )
...
InspColumnDrop = ['GA_LNAME','InspectionID']


RSCOutcomes.drop(InspColumnDrop, axis=1, inplace=True)
RSCOutcomes.rename(columns=InspColumnNames, inplace=True)


RSCScheduled = RSCOutcomes[RSCOutcomes.InspectionStatus=='Scheduled']

RSCSchedInsp_List = RSCScheduled['Consent'].values.tolist()

Consent_col = [
        'RecordNo',
        'B1_PER_SUB_TYPE']

ConsentInfo = pdsql.mssql.rd_sql('SQL2012PROD03',
                   'DataWarehouse', 
                   table = 'D_ACC_Act_Summary',
                   col_names = Consent_col,
                   where_in = {'RecordNo': RSCSchedInsp_List})


OfficerCWMSZone = pdsql.mssql.rd_sql('SQL2012PROD03',
                   'DataWarehouse', 
                   table = 'D_ACC_RMOtoZoneRelation')
...
ConsentHolder.to_csv('r\\fileservices02\ManagedShares\Data\Implementation Support\Python Scripts\OutputData\Scheduled RSC Inspections.csv')
...
# link officer CWMS zone on GA ID
